...
public class ObjectiveTrackerHUD : MonoBehaviour
{
    ...
    private const float CollapseAfter = 0f;
    private Vector2 expandedSize = new(304f, 40f);
    private Vector2 collapsedSize = new(304f, 40f);
    ...
    private void Build(Font f)
    {
        ...
        chapterText = Make("Chapter", 1, FontStyle.Normal, Color.clear, new Vector2(0.5f, 0.5f), 1f);
        titleText = Make("Title", 14, FontStyle.Bold, Color.white, new Vector2(0.5f, 0.5f), 286f);
        keysText = Make("Keys", 1, FontStyle.Normal, Color.clear, new Vector2(0.5f, 0.5f), 1f);
        hintText = Make("Hint", 1, FontStyle.Normal, Color.clear, new Vector2(0.5f, 0.5f), 1f);
    ...
    {
        // The objective is deliberately always visible and compact during gameplay.
    }
    ...
    private void ApplyExpandVisual()
    {
        ...
        if (keysText != null) keysText.gameObject.SetActive(false);
        if (hintText != null) hintText.gameObject.SetActive(false);
        if (chapterText != null) chapterText.gameObject.SetActive(false);
        if (titleText != null)
        {
    ...
    private void RefreshFromBeat()
    {
        ...
        switch (beat.CurrentBeat)
        {
            case ChapterBeatDirector.Beat.Arrival:
                Set(FindFirstObjectByType<PlayerInventory>()?.HasHouseKey == true
